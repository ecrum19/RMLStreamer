...
count_triples_ttl() {
  ...
  # Helper: count triple-like lines in a single TTL file
  _count_triples_file() {
    local f="$1"
    if [[ ! -f "$f" ]]; then
      echo 0
    ...
    # Heuristic: non-comment lines ending with '.'
    grep -E '^[[:space:]]*[^#].*\.[[:space:]]*$' "$f" | wc -l | tr -d ' '
  }

  # If the argument is a directory, iterate over files in it
  if [[ -d "$path" ]]; then
    local total=0
    shopt -s nullglob  # avoid literal '*.ttl' if none exist
  ...
  # If the argument is a single file, just count that file
  if [[ -f "$path" ]]; then
    _count_triples_file "$path"
    return 0
  fi

  # Fallback if path is neither file nor directory
  echo "0"
}
